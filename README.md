...
| `to`        | `string[]` | yes      | Primary recipient addresses (at least one) |
| `add_to`    | `string[]` | no       | Additional recipients; requires `pid` to be set |
...
All recipients across `to` and `add_to` must be distinct (case-insensitive).
...
| Status | Condition |
| ------ | --------- |
| `400`  | Missing/invalid fields, empty `to`, `add_to` without `pid`, or duplicate recipients |
...
Updates a draft message. Only the owner (`from`) may update, and the message must not have been sent yet. Accepts the same body as `POST /api/v1/messages`. Recipients (`to` and `add_to`) are fully replaced.

**Errors:**

| Status | Condition |
| ------ | --------- |
| `400`  | Invalid fields, `add_to` without `pid`, or duplicate recipients |
